...
public class ChargerPaymentInfoRepositoryCustomImpl implements ChargerPaymentInfoRepositoryCustom {
    ...
    public List<PurchaseSalesLineChartBaseDto> searchMonthlyTotalSales(Long companyId, String searchOp,
        ...
        NumberTemplate<Integer> monthTemplate = Expressions.numberTemplate(Integer.class, "MONTH({0})",
                pgSettlmntDetail.settlmntDt);        
    ...
    @Override
    public SalesDashboardDto findPaymentByPeriod(LocalDate startDate, LocalDate endDate, String levelPath,
            boolean isSuperAdmin) {
        QCompany company = QCompany.company;
        QCpInfo cpInfo = QCpInfo.cpInfo;
        QCsInfo csInfo = QCsInfo.csInfo;
        ...
        builder.and(pgSettlmntDetail.settlmntDt.between(startDate, endDate));

        if (!isSuperAdmin && levelPath != null && !levelPath.isEmpty()) {
            builder.and(QueryUtils.hasCompanyLevelAccess(relation, levelPath));
        }

        SalesDashboardDto dto = queryFactory.select(Projections.fields(SalesDashboardDto.class,
                Expressions.numberTemplate(BigDecimal.class,
                    "SUM(CASE WHEN COALESCE({0}, '') = 'SPEEDLOW' THEN {1} ELSE 0 END)", model.cpType,
                    pgSettlmntDetail.depositAmt).as("lowSales"),
                Expressions.numberTemplate(BigDecimal.class,
                    "SUM(CASE WHEN COALESCE({0}, '') = 'SPEEDFAST' THEN {1} ELSE 0 END)", model.cpType,
                    pgSettlmntDetail.depositAmt).as("fastSales"),
                Expressions.numberTemplate(BigDecimal.class,
                    "SUM(CASE WHEN COALESCE({0}, '') = 'SPEEDDESPN' THEN {1} ELSE 0 END)", model.cpType,
                    pgSettlmntDetail.depositAmt).as("despnSales")))
                .from(pgSettlmntDetail)
                .leftJoin(cpInfo).on(pgSettlmntDetail.chargerId.eq(cpInfo.id))
        ...
        if (dto == null) {
            dto = new SalesDashboardDto();
            dto.setLowSales(BigDecimal.ZERO);
            dto.setFastSales(BigDecimal.ZERO);
            dto.setDespnSales(BigDecimal.ZERO);
        } else {
            if (dto.getLowSales() == null) dto.setLowSales(BigDecimal.ZERO);
            if (dto.getFastSales() == null) dto.setFastSales(BigDecimal.ZERO);
            if (dto.getDespnSales() == null) dto.setDespnSales(BigDecimal.ZERO);
        }
