$(document).ready(function () {
    ...
    let selectRow, btnMsg = i18n.pc.buttons.add;
    var id, accountCode;
    ...
    $('#elecBtn').on('click', function (event) {
        ...
        document.getElementById('elecList').innerHTML = '<tr><td colspan="10">' + i18n.pc.messages.nouploaddata + '</td></tr>';
    });

    $('#elecSaveBtn').on('click', function (event) {
        event.preventDefault();
        if (confirmSubmit(i18n.pc.buttons.save)) {
            const DATA = {
    ...
    $('#addBtn').on('click', function (event) {
        ...
        totalContainer.hidden = false;
        totalContainerSec.hidden = true;
        document.getElementById('stationSearchList').innerHTML = '<tr><td colspan="3">' + i18n.pc.messages.nodata + '</td></tr>';
    });

    $('#editBtn').on('click', function (event) {
        event.preventDefault();
        modalCon = true;
        btnMsg = i18n.pc.buttons.edit;
        $('#modalBtn').text(btnMsg);
        $('#purchaseForm')[0].reset();
    ...
    $('#deleteBtn').on('click', function () {
        if (confirmSubmit(i18n.pc.buttons.delete)) {
            $.ajax({
    ...
    $('#modalBtn').on('click', function (event) {
        ...
        if (!form.checkValidity()) {
            alert(i18n.pc.messages.essentialFieldCheckError);
            return;
        }

        if ($('#stationId').val().trim() === '') {
            alert(i18n.pc.messages.searchstationid);
            return;
        }

        // 계정과목:기타
        if ($('#accountCode').val() === 'EFEE' && $('#item').val().trim() === '') {
            alert(i18n.pc.messages.inputitemnm);
            return;
        }

        // 카드결제: 승인번호 확인
        if ($('#paymentMethod').val() === 'PM01' && $('#approvalNo').val().trim() === '') {
            alert(i18n.pc.messages.inputapprovalno);
            return;
        }
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.csList.length === 0) {
                    stationSearchList.innerHTML = '<tr><td colspan="3">' + i18n.pc.messages.nodata + '</td></tr>';
                    return;
                }
                ...
                response.csList.forEach(function (cs) {
                    ...
                    row.innerHTML = `
                        ...
                        <td>${cs.stationId}</td>
                        <td><button type="button" class="btn btn-outline-grey select-station-btn" data-station-id="${cs.stationId}" data-station-name="${cs.stationName}">${i18n.pc.buttons.select}</button></td>
                    `;
